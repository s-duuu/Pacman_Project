...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        corner_state = [self.corners[0], self.corners[1], self.corners[2], self.corners[3]]

        StartState = (self.startingPosition, corner_state)
        return StartState
    ...
    def getNextState(self, state, action):
        ...
        nextx, nexty = int(x + dx), int(y + dy)
        "*** YOUR CODE HERE ***"
        corner_state = [state[1][0], state[1][1], state[1][2], state[1][3]]
        
        for i in range(0,4):
            if (nextx, nexty) == self.corners[i]:
                corner_state[i] = True

        return ((nextx, nexty), corner_state)
...
class FoodSearchProblem:
    ...
    def __init__(self, startingGameState):
        ...
        self.food_pos = startingGameState.getFood().asList()
...
def foodHeuristic(state, problem):
    ...
    for food in food_list:
        candidates.append(mazeDistance(position, food, problem.startingGameState))
        for next in food_list:
            next_food.append(mazeDistance(food, next, problem.startingGameState))
    
    if len(candidates) != 0:
        result += min(candidates) + max(next_food)
    else:
        result += max(next_food)
